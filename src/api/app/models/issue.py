...
class Issue(db.Model):
    ...
    time_spent = db.Column(db.Integer, default=0, nullable=False)  # 已耗时（秒，聚合缓存）
    ...
    time_entries = db.relationship(
        "TimeEntry", backref="issue", cascade="all, delete-orphan", lazy="dynamic",
    )
    ...
    def to_dict(self, include_time_entries: bool = False):
        ...
        if include_time_entries:
            data["time_entries"] = [e.to_dict() for e in self.time_entries] if self.time_entries else []
        return data
...
class TimeEntry(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    issue_id = db.Column(db.String(36), db.ForeignKey("issues.id", ondelete="CASCADE"), nullable=False, index=True)
