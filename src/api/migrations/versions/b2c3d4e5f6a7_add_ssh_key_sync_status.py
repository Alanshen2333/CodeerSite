...
def upgrade() -> None:
    op.add_column('user_ssh_keys', sa.Column('sync_status', sa.String(10), nullable=False, server_default='pending'))
    op.add_column('user_ssh_keys', sa.Column('sync_error', sa.Text(), nullable=True))
    # 已有 gitea_key_id 的记录视为已同步
    op.execute("UPDATE user_ssh_keys SET sync_status = 'synced' WHERE gitea_key_id IS NOT NULL")
