...
class TestAddMember:
    ...
    def test_search_users_by_display_name(self, client, auth_headers):
        client.post("/api/auth/register", json={
            "username": "dnameuser", "email": "dn@example.com", "password": "password123",
        })
        # 设 display_name
        client.patch("/api/auth/me", json={"display_name": "ZoeUnique"}, headers=auth_headers)
        r = client.get("/api/users", query_string={"q": "ZoeUnique"}, headers=auth_headers)
    ...
    def test_add_member_and_list(self, client, auth_headers):
        ...
        r = client.post(f"/api/projects/{slug}/members", json={
            "user_id": bob["id"], "role": "member",
        }, headers=auth_headers)
        assert r.status_code == 201

        members = client.get(f"/api/projects/{slug}/members", headers=auth_headers).get_json()["members"]
    ...
    def test_add_member_dedup_conflict(self, client, auth_headers):
        ...
        client.post(f"/api/projects/{slug}/members", json={
    ...
    def test_non_member_cannot_add(self, client, auth_headers):
        ...
        r = client.post(f"/api/projects/{p['slug']}/members", json={
            "user_id": outsider["id"],
        }, headers=h_outsider)
