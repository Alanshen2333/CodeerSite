...
class TestBadgeCRUD:
    ...
    def test_update_badge(self, client, app):
        ...
        created = client.post(
            "/api/admin/badges", json={"name": "旧名"}, headers=h
        ).get_json()["badge"]
        r = client.patch(
            f"/api/admin/badges/{created['id']}",
            json={
                "description": "新描述",
                "color": "#f5a623",
            },
    ...
    def test_delete_badge(self, client, app):
        ...
        created = client.post(
            "/api/admin/badges", json={"name": "待删"}, headers=h
        ).get_json()["badge"]
    ...
    def test_update_nonexistent_404(self, client, app):
        ...
        r = client.patch(
            "/api/admin/badges/nonexistent-id", json={"name": "x"}, headers=h
        )
...
class TestAwardRevoke:
    ...
    def test_award_success(self, client, app):
        ...
        r = client.post(
            f"/api/admin/badges/{badge['id']}/award",
            json={
                "user_id": target_id,
                "reason": "感谢贡献",
            },
    ...
    def test_revoke_success(self, client, app):
        ...
        # 查获得者列表应为空
        rec = client.get(
            f"/api/admin/badges/{badge['id']}/recipients", headers=h
        ).get_json()
...
class TestConditionEvaluator:
    ...
    def test_invalid_conditions_are_false(self):
        ...
        assert self._check("questions = 1", questions=1) is False
        assert (
            self._check("questions >= 1 and answers >= 1", questions=1, answers=1)
            is False
        )
        ...
        # 注入尝试
        assert (
            self._check("questions >= 1; __import__('os').system('rm -rf /')") is False
        )
